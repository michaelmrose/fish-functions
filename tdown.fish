function tdown
    ...
    if not test -f ~/sounds/alarmvids/"$title".mp4
        ffmpeg -f lavfi -i color=c=black:s=640x480:d=5 -i ~/sounds/alarm.mp3 -vf "drawtext=fontfile=/usr/share/fonts/TTF/DejaVuSans-Bold.ttf:text=\'$title\':fontsize=24:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2" -shortest ~/sounds/alarmvids/"$title".mp4
    end
