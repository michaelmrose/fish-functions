function get-sched-html
    ...
    # curl -c ~/.cjar.txt -L --data "j_username=mmr1135&j_password=Hdac23456&j_storenumber=4716&j_userbelongsto=Store&language=en_US&callingProgram=lmslLaborMgtTools&uiLoc=ext&appendSessionID=false&myLocation=Store&storeUser=true&action=Login&successUrl=$successUrl" $url
    curl -c ~/.cjar.txt -L --data "j_username=mmr1135&j_password=$hdpw&j_storenumber=4716&j_userbelongsto=Store&language=en_US&callingProgram=lmslLaborMgtTools&uiLoc=ext&appendSessionID=false&myLocation=Store&storeUser=true&action=Login&successUrl=$successUrl" $url
end
